...
XBT_LOG_NEW_DEFAULT_CATEGORY(cloud_service,"Log category for HTCondor Service");
...
HTCondorService::HTCondorService(const std::string &hostname,
        ...
        ComputeService("HTCondor_service", "HTCondor_service", supports_standard_jobs, supports_pilot_jobs,
 ...
 */
bool HTCondorService::createVM(const std::string &pm_hostname,
 ...
 */
void HTCondorService::submitStandardJob(StandardJob *job, std::map <std::string, std::string> &service_specific_args) {
        ...
        S4U_Mailbox::putMessage(
                ...
                new ComputeServiceSubmitPilotJobRequestMessage(
                        answer_mailbox, job, this->getPropertyValueAsDouble(
                                HTCondorServiceProperty::SUBMIT_PILOT_JOB_REQUEST_MESSAGE_PAYLOAD)));
    } catch (std::shared_ptr <NetworkError> &cause) {
        throw WorkflowExecutionException(cause);
    }
 ...
 */
void HTCondorService::processCreateVM(const std::string &answer_mailbox,
        ...
        if (simgrid::s4u::Host::by_name_or_null(vm_hostname) == nullptr) {
            ...
            S4U_Mailbox::dputMessage(
                    ...
                    new CloudServiceCreateVMAnswerMessage(
                            true,
                            this->getPropertyValueAsDouble(HTCondorServiceProperty::CREATE_VM_ANSWER_MESSAGE_PAYLOAD)));
        } else {
            S4U_Mailbox::dputMessage(
                    answer_mailbox,
                    new CloudServiceCreateVMAnswerMessage(
 ...
 * @throw std::runtime_error
 */
void HTCondorService::processGetNumCores(const std::string &answer_mailbox) {
 ...
 * @throw std::runtime_error
 */
void HTCondorService::processGetNumIdleCores(const std::string &answer_mailbox) {
