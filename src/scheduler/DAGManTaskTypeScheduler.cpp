...
#include "scheduler/DAGManTaskTypeScheduler.h"
#include "PegasusSimulationTimestampTypes.h"

XBT_LOG_NEW_DEFAULT_CATEGORY(dagman_task_type_scheduler, "Log category for HTCondor Scheduler Daemon");
...
namespace wrench {
    namespace pegasus {
         ...
         */
        DAGManTaskTypeScheduler::DAGManTaskTypeScheduler(FileRegistryService *file_registry_service,
                                                         const std::set<StorageService *> &storage_services)
                : DAGManScheduler(file_registry_service, storage_services) {}

        /**
         * @brief Destructor
         */
        DAGManTaskTypeScheduler::~DAGManTaskTypeScheduler() {}
         ...
         */
        void DAGManTaskTypeScheduler::scheduleTasks(const std::set<ComputeService *> &compute_services,
          ...
          for (auto task : tasks) {
            ...
            if (task->getTaskType() == WorkflowTask::TaskType::TRANSFER) {
              ...
              job = (WorkflowJob *)
                      this->getJobManager()->createStandardJob({task},
                                                               file_locations,
                                                               pre_file_copies,
                                                               post_file_copies, {});
