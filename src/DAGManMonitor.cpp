...
namespace wrench {
    namespace pegasus {
         ...
         */
        std::set<std::shared_ptr<StandardJob>> DAGManMonitor::getCompletedJobs() {
        ...
        bool DAGManMonitor::processNextMessage() {
            ...
            if (auto real_event = std::dynamic_pointer_cast<StandardJobCompletedEvent>(event)) {
                std::shared_ptr<StandardJob> job = real_event->standard_job;
         ...
         * @throw std::runtime_error
         */
        void DAGManMonitor::processStandardJobCompletion(std::shared_ptr<StandardJob> job) {
